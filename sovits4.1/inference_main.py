...
def main():
    ...
    # 一定要设置的部分
    parser.add_argument('-m', '--model_path', type=str, default="sovits4.1/logs/44k/G_120000.pth", help='模型路径')
    parser.add_argument('-c', '--config_path', type=str, default="sovits4.1/logs/44k/config.json", help='配置文件路径')
    parser.add_argument('-cl', '--clip', type=float, default=0, help='音频强制切片，默认0为自动切片，单位为秒/s')
    parser.add_argument('-n', '--clean_names', type=str, nargs='+', default=["君の知らない物語-src.wav"], help='wav文件名列表，放在raw文件夹下')
    parser.add_argument('-t', '--trans', type=int, nargs='+', default=[0], help='音高调整，支持正负（半音）')
    parser.add_argument('-s', '--spk_list', type=str, nargs='+', default=['刻晴[中]'], help='合成目标说话人名称')
    ...
    parser.add_argument('-cr', '--cluster_infer_ratio', type=float, default=0.5, help='聚类方案或特征检索占比，范围0-1，若没有训练聚类模型或特征检索则默认0即可')
    ...
    # 浅扩散设置
    parser.add_argument('-dm', '--diffusion_model_path', type=str, default="sovits4.1/logs/44k/diffusion/model_50000.pt", help='扩散模型路径')
    parser.add_argument('-dc', '--diffusion_config_path', type=str, default="sovits4.1/logs/44k/diffusion/config.yaml", help='扩散模型配置文件路径')
    ...
    if cluster_infer_ratio != 0:
        if args.cluster_model_path == "":
            if args.feature_retrieval:  # 若指定了占比但没有指定模型路径，则按是否使用特征检索分配默认的模型路径
                args.cluster_model_path = "sovits4.1/logs/44k/feature_and_index.pkl"
            else:
                args.cluster_model_path = "sovits4.1/logs/44k/kmeans_10000.pt"
    ...
    for clean_name, tran in zip(clean_names, trans):
        ...
        for spk in spk_list:
            ...
            res_path = f'{os.path.splitext(clean_name)[0]}_{spk}.{wav_format}'
